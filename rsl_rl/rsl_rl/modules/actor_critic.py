...
# Actor-Critic策略网络基类实现
# 共享MLP骨干，分离Actor（输出动作均值/标准差）和Critic（输出状态价值V）
# 支持独立观测归一化、状态相关噪声、scalar/log两种标准差参数化方式
...
class ActorCritic(nn.Module):
    ...
    Actor与Critic使用独立MLP，Actor输出高斯分布的动作均值与标准差，
    Critic输出标量价值估计。可选EmpiricalNormalization对观测进行在线归一化。
    """
    ...
    def _update_distribution(self, obs: torch.Tensor) -> None:
        # state_dependent_std模式下Actor同时输出均值和标准差，否则标准差为可学习参数
        if self.state_dependent_std:
