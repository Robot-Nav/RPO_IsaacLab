...
# 随机网络蒸馏（RND）内在奖励模块实现
# 目标网络固定随机初始化不训练，预测器学习拟合目标网络输出
# 内在奖励 = 预测器与目标网络输出的距离，鼓励探索未知状态
# 支持状态归一化、奖励归一化、权重调度（constant/step/linear）
...
class RandomNetworkDistillation(nn.Module):
    ...
    通过预测器拟合随机固定目标网络的输出，将二者差异作为内在奖励驱动探索。
    目标网络永远不更新，预测器在训练中逐步拟合已访问状态，奖励随训练自然衰减。
