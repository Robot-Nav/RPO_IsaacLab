...
# 带注意力编码器的Actor-Critic网络实现
# 通过多头自注意力对感知观测（高度图等）编码，与本体观测融合后送入Actor/Critic MLP
# 可选Critic估计分支（辅助任务）与观测编码器（学习低维潜在表示），辅助损失用于训练Critic估计
...
class ActorCriticAttnEnc(nn.Module):
    """基于注意力编码器的Actor-Critic网络。

    Actor/Critic前先用AttentionEncoder处理感知观测获得嵌入向量，
    与本体观测拼接后送入MLP。可选分支：Critic状态估计辅助任务、观测低维编码器。
    """
