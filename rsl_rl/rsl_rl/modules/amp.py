...
# AMP对抗模仿学习判别器与配置解析
# 通过判别策略生成动作与参考动作分布差异，输出风格奖励融入PPO训练
# 支持GAN/LSGAN/WGAN损失，提供梯度惩罚稳定训练，并暴露风格奖励计算接口
import torch
...
class AMPDiscriminator(nn.Module):
    ...
    输入策略生成动作与参考动作的观测序列，输出标量判别分数。
    支持GAN/LSGAN/WGAN三种损失，并提供：
    - 风格奖励：将判别分数映射为对策略的奖励信号
    - 任务/风格奖励插值：平衡任务目标与动作风格先验
    - 梯度惩罚：约束判别器Lipschitz连续性，稳定训练
    """
    def __init__(self, 
            disc_obs_dim: int,
            disc_obs_steps: int,
