...
# 基于CNN的Actor-Critic网络，支持图像（2D）与本体（1D）观测混合输入
# 各2D观测组独立CNN编码后展平，与1D观测拼接送入Actor/Critic MLP
# 不支持state_dependent_std时退化为标准MLP头
...
class ActorCriticCNN(ActorCritic):
    """支持2D图像观测的Actor-Critic。每个2D观测组配置独立CNN编码器，
    展平后与1D本体观测拼接送入Actor/Critic MLP。"""
    def __init__(
        self,
        obs: TensorDict,
