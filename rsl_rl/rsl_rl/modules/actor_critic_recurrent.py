...
# 循环网络Actor-Critic实现
# 在Actor/Critic的MLP前加入LSTM/GRU记忆模块，处理部分可观测性与历史依赖
# Actor与Critic使用独立记忆模块，支持dones时步重置隐藏状态
...
class ActorCriticRecurrent(nn.Module):
    """带循环记忆模块的Actor-Critic。

    Actor和Critic分别接入独立Memory（LSTM/GRU），将观测编码为隐藏状态后送入MLP，
    支持部分可观测决策。dones信号触发隐藏状态按环境重置。
    """
