...
# 循环缓冲区实现（用于AMP参考动作采样）
# 多环境共享一个环形缓冲区，每个环境独立维护push计数与有效长度
# 支持LIFO顺序检索、按batch_id局部重置、按需采样mini-batch（线性索引→batch/时间双索引）
...
class CircularBuffer:
    ...
    每个环境独立的环形缓冲区，append按指针循环写入，首条数据会广播到所有槽位初始化。
    通过__getitem__按LIFO检索历史数据，支持mini-batch随机采样（用于AMP判别器训练）。
    """
    ...
    def mini_batch_generator(self, fetch_length: int, num_mini_batches: int, num_epochs=8):
