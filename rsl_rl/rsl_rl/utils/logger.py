...
# 训练日志记录器基类：管理tensorboard/wandb/neptune后端，
# 维护奖励与episode长度滑动窗口，记录git diff与配置
...
class Logger:
    ...
    中文说明：训练日志基类，管理奖励/episode长度滑动缓冲、RND内外奖励记录、
    后端writer选择与git代码状态保存，分布式训练时仅rank0进程写入日志。
    """
    ...
    def process_env_step(
        ...
        中文说明：将环境步数据累加到episode缓冲，done时把完整episode统计推入滑动窗口。
        支持RND场景下内外奖励分别统计。
        """
    ...
    def log(
        ...
        中文说明：每轮迭代输出训练指标到writer与控制台，包含损失、学习率、动作噪声、
        奖励/episode长度统计、FPS、ETA等，并处理episode extras（带"/"前缀的key按命名空间记录）。
        """
        if self.log_dir is not None and not self.disable_logs:
            ...
            # episode extras按key聚合多步值后取均值写入writer
            extras_string = ""
            if self.ep_extras:
    ...
    def _store_code_state(self) -> None:
        ...
        中文说明：保存实验涉及代码仓库的git status与diff到日志目录，便于复现实验时的代码版本追溯。
        """
        if self.log_dir is not None and not self.disable_logs:
