...
# Neptune日志后端：继承torch SummaryWriter，
# 通过环境变量NEPTUNE_API_TOKEN/NEPTUNE_USERNAME初始化
...
class NeptuneSummaryWriter(SummaryWriter):
    ...
    中文说明：Neptune后端writer，name_map处理Neptune不兼容的tag字符（如"/"结尾路径），
    通过_map_path将带斜杠的tensorboard路径映射为合法的Neptune路径。
    """
