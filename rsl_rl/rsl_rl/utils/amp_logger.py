...
# AMP训练日志记录器：在Logger基础上扩展风格奖励(style reward)
# 与总奖励(total reward)滑动窗口记录
import git
import os
import pathlib
import statistics
...
class LoggerAMP(Logger):
    ...
    中文说明：AMP专用日志记录器，扩展total/style奖励滑动缓冲与对应记录。
    style奖励按episode归一化（除以最大episode时长秒数）以消除长度差异。
    """
    ...
    def process_env_step(
        ...
        if self.log_dir is not None:
            ...
            if style_rewards is not None and total_rewards is not None:
                # 无done时也计算style均值（按秒归一化），有done时记入episode extras
