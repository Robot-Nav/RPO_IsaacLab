...
# 通用工具函数集合：参数获取、激活函数解析、优化器解析、
# 轨迹切分padding、可调用对象解析、观测组解析
...
def split_and_pad_trajectories(
    ...
    中文说明：按done标志切分轨迹并补零对齐，返回padding后的轨迹张量与有效位mask。
    输入维度顺序约定为 [时间步, 环境数, 额外维度]，处理RNN训练时变长轨迹批次的关键步骤。
    """
    dones = dones.clone()
    dones[-1] = 1  # 强制最后一个时间步为done，保证尾段轨迹被切出
    # 转置为 (环境数, 时间步, ...) 以便flatten后按时间顺序拼接
    flat_dones = dones.transpose(1, 0).reshape(-1, 1)
    # 通过相邻done索引差值计算每条轨迹长度
...
def unpad_trajectories(trajectories: torch.Tensor | TensorDict, masks: torch.Tensor) -> torch.Tensor | TensorDict:
    ...
    中文说明：split_and_pad_trajectories的逆操作，按mask去除padding并恢复 [时间步, 环境数, ...] 布局。
    """
    # 转置后用mask索引有效位，再reshape回原始时间步维度
    return (
...
def resolve_callable(callable_or_name: type | Callable | str) -> Callable:
    ...
    """
    # 中文说明：支持四种解析格式：直接传入可调用对象、冒号显式路径、点号隐式路径、rsl_rl内简单名查找
    ...
    # 冒号分隔的显式路径：module.path:Attr.Nested，最推荐用法
...
def resolve_obs_groups(
    ...
    """
    # 中文说明：校验观测组配置并将缺失的默认观测集（如critic）回退到policy观测或同名环境观测组
    # policy观测集为必填项，缺失时报错或尝试用同名环境观测组补齐
