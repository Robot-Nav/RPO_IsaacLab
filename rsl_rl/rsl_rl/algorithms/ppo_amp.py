# PPO-AMP算法实现：在PPO基础上集成对抗模仿学习（Adversarial Motion Prior）
# 通过判别器区分策略生成动作与参考动作，输出风格奖励与任务奖励插值后作为最终奖励
# 支持GAN/LSGAN/WGAN三种判别器损失，以及梯度惩罚稳定训练
...
class PPOAMP(PPO):
    """PPO with Adversarial Motion Prior (AMP).

    在PPO基础上引入动作风格判别器，将参考动作分布作为先验融入策略学习，
    生成的风格奖励与任务奖励按系数插值，使策略同时满足任务目标和风格约束。
    """
    ...
    def update(self) -> dict[str, float]:
        ...
        for samples, disc_obs_batch, disc_demo_obs_batch in zip(generator, disc_obs_generator, disc_demo_obs_generator):
            ...
            # AMP判别器损失：使用归一化后的判别器观测分别前向，最小化策略样本得分、最大化参考样本得分
            with torch.no_grad():
            ...
            if self.loss_type == LossType.GAN:
                # 标准GAN损失：BCEWithLogits，标签策略样本0、参考样本1
                ...
                # LSGAN损失：MSE形式，标签策略样本-1、参考样本1，缓解GAN梯度消失
                ...
                # WGAN损失：参考样本得分均值减策略样本得分均值，逼近Wasserstein距离
            ...
            # 梯度惩罚：约束参考样本处梯度范数，满足Lipschitz连续性
