...
# 策略蒸馏实现：将特权信息教师网络的行为克隆到学生网络
# 教师可观测全部特权信息，学生仅观测本体感知，通过MSE/Huber损失进行行为克隆
# 支持梯度截断长度累积（BPTT截断）以处理循环网络中的长序列梯度爆炸/消失
...
class Distillation:
    ...
    通过行为克隆让仅拥有本体感知的学生网络模仿拥有特权观测的教师网络，
    实现特权信息蒸馏，用于部署阶段消除对特权观测的依赖。
    """
