...
# 卷积神经网络（CNN）基础组件
# 由Conv2d层 + 可选归一化 + 激活 + 可选MaxPool堆叠构成
# 支持自动padding计算、全局池化、Kaiming初始化，输出可展平或保留空间维度
...
class CNN(nn.Sequential):
    ...
    每层由Conv2d + 可选BatchNorm/LayerNorm + 激活 + 可选MaxPool组成。
    末尾可选全局池化（AdaptiveMax/AvgPool）与Flatten。padding按"samen"策略自动计算。
    """
