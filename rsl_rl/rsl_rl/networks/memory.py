...
# 循环记忆模块（Memory）实现
# 封装nn.GRU/nn.LSTM，支持隐藏状态保存与按环境dones重置
# 提供batch模式（训练时用外部传入hidden_state）与推理模式（自动维护内部状态）
...
GRU为单tensor，LSTM为(hidden, cell)二元组，None表示尚未初始化。
"""
...
class Memory(nn.Module):
    ...
    支持两种模式：batch模式（训练时由外部传入hidden_state用于BPTT）与推理模式（内部维护上一步状态）。
    dones信号触发对应环境的隐藏状态清零，实现episode边界正确分离。
    """
