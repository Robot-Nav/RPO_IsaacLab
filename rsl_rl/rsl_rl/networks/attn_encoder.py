# 注意力编码器实现：将本体感知与高度图扫描融合为低维嵌入
# CNN提取局部空间特征，与位置编码、原始高度值拼接为点级特征
# 多头自注意力以本体感知为query，点级特征为key/value，输出聚合嵌入
import torch
...
class AttentionEncoder(nn.Module):
    ...
    CNN对高度图扫描提取空间特征，与坐标位置编码和原始高度拼接为点级特征。
    本体感知经线性投影为query，点级特征为key/value，通过多头注意力聚合输出嵌入向量。
    返回嵌入与注意力权重（便于可视化/调试）。
    """
