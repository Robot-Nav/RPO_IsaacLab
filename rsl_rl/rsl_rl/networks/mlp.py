...
# 多层感知机（MLP）基础组件
# 由Linear层与激活函数交替堆叠构成，支持hidden_dims=-1自动推断、
# output_dim支持tuple（自动reshape）、可选末层激活与正交初始化
...
class MLP(nn.Sequential):
    ...
    由Linear与激活函数交替堆叠构成。hidden_dims中-1表示沿用输入维度，
    output_dim支持tuple（自动reshape末层输出），可选末层激活与正交初始化。
