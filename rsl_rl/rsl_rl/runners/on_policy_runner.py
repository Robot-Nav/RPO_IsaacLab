...
# 同策略训练运行器（OnPolicyRunner），PPO主循环实现
# 负责环境交互、rollout采集、算法更新、日志记录、模型保存/加载的完整流程
# 支持多GPU分布式训练、RND内在奖励、对称性增强等可选组件
...
class OnPolicyRunner:
    ...
    PPO主循环：rollout采集→计算回报与优势→策略更新→日志记录→模型保存。
    支持多GPU分布式训练、RND内在奖励、对称性增强，并通过Logger统一管理日志输出与外部追踪。
    """
    ...
    def _construct_algorithm(self, obs: TensorDict) -> PPO:
        ...
        actor_critic: ActorCritic | ActorCriticRecurrent | ActorCriticCNN | ActorCriticAttnEnc = actor_critic_class(
