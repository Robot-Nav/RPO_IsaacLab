...
# 蒸馏训练运行器实现
# 继承OnPolicyRunner，覆盖构造算法流程为构建StudentTeacher网络 + Distillation算法
# 训练前强制要求加载教师模型，RND配置自动置空
...
class DistillationRunner(OnPolicyRunner):
    ...
    训练前需加载教师模型权重，否则抛出异常。算法构造构建StudentTeacher策略与Distillation算法，
    并显式将RND配置置空（蒸馏不使用内在奖励）。
    """
