...
# AMP训练运行器实现
# 继承OnPolicyRunner，扩展PPO-AMP训练循环：判别器观测缓冲区管理、风格奖励记录、判别器模型保存/加载
# 支持AMP判别器独立优化器、归一化器与训练/评估模式切换
...
class AMPRunner(OnPolicyRunner):
    ...
    在OnPolicyRunner基础上集成AMP判别器，扩展判别器观测循环缓冲区、
    风格奖励记录、判别器模型/优化器/归一化器的保存加载逻辑。
    """
