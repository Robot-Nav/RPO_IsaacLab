# 图像噪声模型集合，模拟深度/RGB传感器各类成像缺陷与传输延迟
# 用于域随机化训练，提升策略在真实传感器噪声下的鲁棒性，支撑sim2real迁移
...
from robolab.utils.buffers import AsyncDelayBuffer
...
class ImageNoiseModel:
    ...
    """
    # 图像噪声模型基类，子类需实现__call__完成具体噪声注入
    # 有状态噪声（如延迟、传感器掉线）通过reset接口按环境复位
...
def depth_contour_noise(
    ...
    """
    # 深度图轮廓处会产生无效像素，模拟立体匹配在深度跳变边缘的失真
    # 用8方向3x3卷积核检测梯度，max_pool扩张后置零
    device = cfg.device
...
def depth_stero_noise(
    ...
    # 模拟双目立体相机（如D435i）的深度噪声特性
    # 远处像素噪声大、近处噪声小，过近区域按垂直列是否全部过近分为full_block和half_block两种伪影模式
    device = cfg.device
    N, H, W, _ = data.shape
    ...
    # 远距离点噪声标准差更大，立体匹配基线限制导致远距离深度估计误差放大
    ...
    # 判断过近区域是否整列垂直方向都过近（>60%），区分full_block与half_block两种D435i真实成像模式
    ...
    # full_block: 整列过近区域叠加条带状伪影，模拟相机最小工作距离内的无效输出
    ...
    # half_block: 非整列过近区域以spark形式随机出现亮点，模拟立体匹配在边缘的零散失效
    half_block_spark = (
...
def depth_sky_artifact_noise(
    ...
    # 模拟相机指向天空/天花板时的深度伪影，立体匹配在无纹理区域失效产生条带
    # 通过列方向累积判断像素上方是否全部过远，识别"看向天空"的区域
    device = cfg.device
    N, _, H, W = data.shape
    ...
    def _recognize_top_down_seeing_sky(too_far_mask):
        # 沿高度方向累积求和，若某像素上方的过远像素数超过其行号，则判定为看向天空
...
class LatencyNoiseModel(ImageNoiseModel):
    # 传感器延迟噪声模型，按各环境独立的延迟分布采样时间滞后步数
    # 支持均匀/正态/离散选择/常数四种延迟分布，以及按步数或概率两种重采样频率
    # 延迟值通过AsyncDelayBuffer作用于观测数据，模拟真实传感器的传输与处理时延
    def __init__(self, cfg: LatencyNoiseCfg, num_envs, device):
        ...
        # 各环境独立的步数计数器与上次重采样步数，用于按频率触发重采样
    ...
    def __call__(self, data, cfg, env_ids: torch.Tensor | Sequence[int]):
        # 推进步数计数器，按重采样频率更新各环境的延迟值，最后从缓冲区取出延迟后的数据
    ...
    def _generate_resample_intervals(self, env_ids: Sequence[int] | None = None):
        ...
        # 在基础步数上叠加随机偏移，使各环境重采样时刻错开，避免同步尖峰
...
def _add_depth_artifacts(
    ...
    """
    # 在深度图上随机生成矩形条带伪影，模拟立体相机在无效区域的块状输出
    # 核心技巧：用grid_sample批量生成各伪影的矩形掩码，再用einsum按环境聚合，避免逐个绘制
    N, H, W, _ = data.shape
    ...
    # 仅在有效像素（depth>0）上以artifacts_prob概率撒种子点
    ...
    # final_masks: (N, H, W)
    # 用einsum将各伪影按所属环境求和，得到每张图的最终伪影掩码
...
def stereo_fusion_noise(
    ...
    # 模拟立体匹配一致性检查产生的空洞：遮挡边缘（视差梯度大）与弱纹理区域（局部方差小）处随机置为hole_value
...
def perlin_noise(
    ...
    # 分形布朗运动噪声：多octave叠加不同频率的高斯白噪声并双线性上采样
    # 频率按lacunarity倍增、幅度按persistence衰减，最后归一化后叠加到图像
    if random.random() > cfg.apply_probability:
        return data
...
class SensorDeadNoiseModel(ImageNoiseModel):
    # 传感器掉线噪声模型，模拟传感器死机重启期间输出冻结帧
    # 以dead_probability概率触发掉线，掉线期间持续dead_frames帧输出上一次有效数据
    # 掉线结束后缓冲区恢复正常刷新
