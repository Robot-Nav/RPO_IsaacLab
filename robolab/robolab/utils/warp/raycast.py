# 分组网格射线投射，按碰撞组对射线与网格求交，支持网格世界变换
# 用于深度传感器仿真：射线按所属碰撞组仅与指定网格集合求交，避免无关碰撞
import numpy as np
import torch
...
def raycast_mesh_grouped(
    ...
    """
    # 射线/网格按碰撞组匹配，可选返回命中距离、法线、面索引、网格索引
    ...
    # reshape the tensors
    # 将射线数据搬到网格所在设备并展平为(N,3)，便于kernel按线程索引
    ...
    # warp的transform使用xyzw四元数，需从wxyz转换
    ...
    # 未启用返回项时传入长度为1的占位数组，满足kernel参数签名要求
    wp.launch(
