# 圆柱体空间网格管理，将圆柱体按AABB分桶到均匀网格以加速穿透查询
# 用于机器人身体碰撞检测：批量查询点集相对场景圆柱障碍物的穿透偏移
import numpy as np
import torch
...
class CylinderSpatialGrid:
    ...
    """
    # 圆柱体空间网格索引，构建时将每个圆柱体注册到其AABB覆盖的所有网格单元
    # 查询时通过点所在单元快速定位候选圆柱体，避免O(N*M)暴力遍历
    ...
    def _compute_bounding_box(self):
        # 包含所有圆柱体端点的整体AABB，向外扩张r_max以容纳圆柱半径
        ...
        # 按各轴跨度比例自适应分配网格分辨率，使网格单元尽量接近立方体
    ...
    def _create_grid(self):
        # 将每个圆柱体注册到其AABB覆盖的所有网格单元，构建CSR格式的网格索引
        ...
        # CSR格式：cell_offsets给出每单元在cell_indices中的起止位置，便于GPU单次访存
    ...
    def get_points_penetration_offset(self, points: torch.Tensor) -> torch.Tensor:
        ...
        """
        # 启动warp kernel并行计算每个点相对圆柱体集合的最大穿透偏移
