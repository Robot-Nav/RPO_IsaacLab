...
# 分组射线投射相机配置
# 在 GroupedRayCasterCfg 基础上增加相机特有的 offset、data_types、深度裁剪与针孔模式配置
...
class GroupedRayCasterCameraCfg(GroupedRayCasterCfg):
    ...
    分组射线投射相机配置：绑定 GroupedRayCasterCamera 运行时类，
    通过 offset 定义传感器相对父系的位姿，pattern_cfg 定义针孔相机射线分布。
    """
    ...
    class OffsetCfg:
        ...
        传感器相对父系的位姿偏移，convention 决定坐标轴约定（ROS/OpenGL/World）。
        """
