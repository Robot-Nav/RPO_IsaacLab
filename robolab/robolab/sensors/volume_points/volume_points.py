# 体积点云传感器，在刚体表面/体积内采样 3D 点云用于碰撞检测与穿透量计算
# 通过 points_generator 在体坐标系生成点云 pattern，再变换到世界系与虚拟障碍物求交
# 输出每个点的世界位姿、速度以及相对虚拟障碍物的穿透偏移量（penetration_offset）
...
class VolumePoints(SensorBase):
    ...
    体积点云传感器：在指定刚体集合上采样 3D 点云，并计算点云相对虚拟障碍物的穿透偏移。
    用于强化学习中以软约束方式惩罚机器人自碰撞或与环境的穿透。
    """
    ...
    def register_virtual_obstacles(self, virtual_obstacles: dict[str, Any]) -> None:
        ...
        """
        # 注册虚拟障碍物字典，每个障碍物需实现 get_points_penetration_offset 接口
    ...
    def _initialize_impl(self):
        ...
        # 从配置的 prim_path 叶子名解析出所有刚体名，并构造 PhysX glob 表达式
        ...
        # view.count 是所有环境刚体总数，除以环境数得到单环境刚体数
        ...
        # 调用配置中的点云生成器生成本地 pattern (P, 3)，所有刚体共享同一 pattern
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        # 全量更新时用 slice(None) 替换 env_ids，避免每次构建索引张量
    ...
    def _refresh_volume_points(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        # PhysX 返回 xyzw 格式四元数，转换为 wxyz 以匹配 isaaclab 工具函数约定
        ...
        # 将本地 pattern 通过刚体位姿变换到世界系，按 (N_, B) 扁平后批量变换再 reshape 回 (N_, B, P, 3)
        ...
        # 点速度 = 刚体质心速度 + 角速度 × (点位置 - 质心位置)，即刚体运动学公式
    ...
    def _refresh_penetration_offset(self, env_ids: Sequence[int]) -> None:
        ...
        # 对每个虚拟障碍物求穿透偏移，取穿透深度最大的作为该点的最终结果
        # 同一点可能同时穿入多个障碍物，取最深穿透用于奖励/惩罚计算
        ...
        for virtual_obstacle in self._virtual_obstacles.values():
            ...
            # 按 (N_*B*P) 扁平后送入障碍物接口，再 reshape 回原形状
    ...
    def _debug_vis_callback(self, event):
        ...
        # 无穿透点时强行补一个占位 True，避免 marker_indices 全 0 时可视化混淆
