# 高度场地形生成函数集合，所有函数输出 int16 高度场数组并经装饰器转换为 mesh
# 通用模式：1) 难度区间参数线性插值为具体值 2) 物理量转离散像素（除以 horizontal/vertical_scale）
# 3) 中心保留 platform 供机器人生成 4) 可选叠加 Perlin 噪声增强真实感
...
def generate_perlin_noise(difficulty: float, cfg: hf_terrains_cfg.PerlinPlaneTerrainCfg) -> np.ndarray:
    ...
    Args:
        difficulty: 难度系数 [0, 1]，用于在 noise_scale 为区间时线性插值。
        cfg: Perlin 噪声地形配置，提供尺寸、频率、分形参数等。

    Returns:
        int16 高度场数组，已除以 vertical_scale 转为离散高度刻度。
    """
    # noise_scale 为区间时按难度线性插值，否则直接使用标量
    noise_scale = (
        cfg.noise_scale
...
def perlin_pyramid_sloped_terrain(difficulty: float, cfg: hf_terrains_cfg.PerlinPyramidSlopedTerrainCfg) -> np.ndarray:
    ...
    # 坐标归一化到 [-1, 1] 三角波：边缘为 0、中心为 1，相乘后形成四棱锥
    ...
    # 取平台角落处的高度作为 clip 上下界，从而保留平台外缘以下至平台高度区间的部分
...
def perlin_stepping_stones_terrain(
    ...
    if length_pixels >= width_pixels:
        while start_y < length_pixels:
            ...
            # 每行首块石头从 0 到 stop_x，制造行间错位效果，避免规则的网格排布
...
def perlin_tilt_terrain(difficulty: float, cfg: hf_terrains_cfg.PerlinTiltTerrainCfg):
    ...
    if wall_opening_angle > 0:
        # 带角度的开口：开口宽度随长度线性扩展（tan(angle)），形成梯形/喇叭形门洞
...
def perlin_tilted_ramp_terrain(difficulty: float, cfg: hf_terrains_cfg.PerlinTiltedRampTerrainCfg):
    ...
    if switch_spacing > 0.0:
        # 交替斜坡模式：沿 y 方向以 switch_spacing 为周期，左右两侧交替出现斜坡
        # overlap_size 控制左右斜坡在中央的重叠量，避免出现完全平坦的空隙
        ...
        while end_y_px < length_pixels and end_y_px <= tilt_length:
            if tilt_left:
                # 左侧斜坡：从 width_empty_space 升至 start_left_px，linspace 生成高度剖面
                ...
                # 右侧斜坡：从 start_right_px 升至 width_pixels - width_empty_space
        ...
        # 两侧对称斜坡：左右两侧同时布置斜坡，形成中间高、两侧低的对称剖面
