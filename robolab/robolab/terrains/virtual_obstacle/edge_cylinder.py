# 边缘圆柱虚拟障碍物实现：从地形网格检测尖锐边缘，构造虚拟圆柱用于穿深计算
# 基类 EdgeCylinder 用面邻接角判定尖锐边；子类用不同算法合并/拟合边段：
#   Plucker（Plücker 坐标合并共线）、Ransac（DBSCAN+RANSAC 拟合）、Greedyconcat（贪心串联）
#   Ray（射线投射+Canny 边缘检测）、Feature（pyvista 特征边提取）
# 输出统一为 (num_edges, 6) 端点数组，并构建 CylinderSpatialGrid 加速穿深查询
...
from robolab.utils.warp.cylinder import CylinderSpatialGrid
...
class EdgeCylinder(VirtualObstacleBase):
    ...
    基于网格面邻接角检测尖锐边：相邻两面夹角超过 angle_threshold 视为尖锐边，
    收集端点坐标后交由子类的 process_edges 进一步合并/拟合。
    """
    ...
    def generate(self, mesh: trimesh.Trimesh, device="cpu") -> None:
        ...
        # 构建圆柱空间网格：将每条边扩展为半径 cylinder_radius 的圆柱，按 AABB 分桶加速穿深查询
...
class PluckerEdgeCylinder(EdgeCylinder):
    ...
    Plücker 坐标法合并共线边段：用 (方向, 矩量) 6D 表示判定共线，
    再以参数 t 上的事件排序提取未被覆盖的区间，输出合并后的边段。
    """
    ...
    def process_edges(self, edge_coords: np.ndarray) -> np.ndarray:
        ...
        Returns:
            A np array of processed edge coordinates.
        """
        # Plücker 坐标由方向 d 与矩量 m=cross(d,p0) 构成，二者共同确定一条空间直线
        ...
        # 规范化方向：将首非零分量置正，使方向相反的同一条边映射到同一表示
        ...
        for group in same_para_groups:
            ...
            # 扫描线法：端点作为事件，起点 +1、终点 -1，count==1 的区间为单一覆盖段
            events = []
            for t in t_group:
                if t[0] < t[1]:
                    events.append((t[0], 1))
...
class RansacEdgeCylinder(EdgeCylinder):
    ...
    RANSAC 法：DBSCAN 对端点聚类后，每簇并行随机抽样两点拟合直线，
    取内点最多者为代表线段，循环直至剩余点不足。
    """
    ...
    def process_edges(self, edge_coords: np.ndarray) -> np.ndarray:
        ...
        # DBSCAN 聚类：将空间相近的端点归为一簇，每簇对应一条候选边
...
class GreedyconcatEdgeCylinder(EdgeCylinder):
    ...
    贪心串联法：构建顶点邻接表后，从随机顶点出发沿方向夹角阈值双向延伸成折线，
    再以最大垂距 < 0.05 为判据将折线简化为直线段。
    """
    ...
    def process_edges(self, edge_coords: np.ndarray) -> np.ndarray:
        ...
        while available_edges:
            ...
            # 折线简化：从前往后找首条满足最大垂距 < 0.05 的直线段，长度达标则输出
...
class RayEdgeCylinder(VirtualObstacleBase):
    ...
    射线投射法：多虚拟相机位姿发射网格射线得深度图与法向图，分别跑 Canny 边缘检测，
    异或合并得候选边缘点，再经 DBSCAN 聚类 + RANSAC 拟合为线段。
    """
    ...
    def generate(self, mesh: trimesh.Trimesh, device="cpu") -> None:
        ...
        # 射线网格尺寸按地形短边减去高度偏移，确保覆盖整个地形投影
        pattern_cfg.size = [
            min(mesh_size[:2]) - self.cfg.ray_offset_pos[-1],
            min(mesh_size[:2]) - self.cfg.ray_offset_pos[-1],
        ]

        # 四组轴角定义四个对角虚拟相机位姿，从不同方向覆盖地形
        ...
        # 未命中射线的深度为 NaN/inf，用有效点的线性插值填充以避免 Canny 出现伪边缘
        ...
        # 多线程并行对各相机跑 Canny：深度图捕捉高度突变，法向图捕捉朝向突变
        ...
        def process_camera(i):
            ...
            # 异或合并两类边缘：仅深度突变或仅法向突变的点保留，去除两者共同的噪声
...
class FeatureEdgeCylinder(VirtualObstacleBase):
    ...
    特征边法：借 pyvista extract_feature_edges 直接提取边界边、非流形边、特征边，
    feature_angle 控制特征边角度阈值，逐线段解析端点对输出。
    """
    ...
    def generate(self, mesh: trimesh.Trimesh, device="cpu") -> None:
        ...
        points = mesh.vertices.astype(np.float32)
        indices = mesh.faces.astype(np.int32)
        # pyvista PolyData 需要面片前缀 3 表示三角形顶点数
        ...
        if edges.n_cells > 0:
            ...
            edge_coords = []
            i = 0
            # pyvista 线段格式为 [n, idx0, idx1, ..., n, idx0, idx1, ...]，逐段解析
            while i < len(lines):
                num_pts = int(lines[i])
                if num_pts == 2:
...
def process_camera_edges(i, depth_image, normal_image, cfg):
    """对单个相机的深度图与法向图分别跑 Canny 边缘检测。

    深度图先归一化到 0-255 再检测；法向图需先修复接近白色的无效像素再转灰度检测。
    """
    ...
    # 近白色像素多为未命中射线的填充值，用 inpaint 修复避免 Canny 误检
...
def _fit_segments_for_group(grp, cfg_dict):
    """对单簇点集反复 RANSAC 拟合直线段，直至剩余点不足。

    每轮随机抽两点定直线，统计垂距小于阈值的内点数，取最优者按参数 t 的极值确定段端点，
    随后将内点从剩余集合剔除进入下一轮。
    """
