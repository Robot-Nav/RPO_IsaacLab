# 课程学习函数：根据速度跟踪表现升降地形难度、动态调整奖励权重
# 核心思路：跟踪好→升难度/放大安全奖励；跟踪差→降难度/回到温和奖励
...
def tracking_exp_vel(
    ...
    # 从command.metrics读取指数核跟踪分数，由command类内部维护滑动窗口
    ...
    # 升级条件：xy和yaw跟踪分都高于上阈值；降级条件：xy低于下阈值（且未升级）
...
def modify_rewards_weight(
    ...
    # 按step_size比例向目标权重靠拢：move_up向final_weight，move_down向init_weight
    # 不直接跳到目标值，而是按比例缩小与目标的差距，保证训练曲线平滑
    ...
    # 返回全体env的均值而非reset批次均值，让训练曲线反映整体趋势
