# 域随机化函数：随机化默认关节位置/射线起点方向/相机外参，模拟标定误差与安装误差提升sim2real
...
from robolab.sensors.grouped_ray_caster import GroupedRayCaster
...
def randomize_default_joint_pos(
    ...
    # 保存名义值便于策略部署时恢复原始默认姿态，与随机化后的默认值区分
    ...
    if offset_distribution_params is not None:
        ...
        # 关节默认位置变化后需同步更新action_manager的_offset，否则joint_pos动作的目标偏移会失配
...
def randomize_ray_offsets(
    ...
    # 每条射线采样独立的6DoF扰动，repeat到num_rays维度后flatten方便批量旋转计算
    ...
    # 平移扰动加到ray_starts，旋转扰动应用到ray_directions（起点旋转通常忽略避免几何畸变）
...
def randomize_camera_offsets(
    ...
    # 读取相机cfg中的offset位姿作为基准，转换到世界系四元数后按env重复
    ...
    # 对x/y/z位置与roll/pitch/yaw欧拉角分别采样并叠加，转回四元数
    ...
    # 将offset变换与view世界位姿合成，得到最终相机世界位姿
