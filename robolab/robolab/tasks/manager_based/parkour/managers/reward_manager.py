...
# 多奖励组管理器：扩展RewardManager支持按组聚合奖励，并为每个env维护独立权重
# 课程学习通过set_term_weight_for_envs动态调整per-env权重，实现按环境难度的差异化奖励
...
class MultiRewardManager(RewardManager):
    ...
    def __init__(self, cfg: MultiRewardCfg, env: ManagerBasedRLEnv):
        ...
        # 准备三类缓冲：episode_sums记录每个term的回合累计；reward_buf按组聚合；termwise_reward_buf记录单步逐项值
        ...
        # per-env权重缓冲：每个term对每个env维护独立权重，初始化为cfg中的标量weight
        # 课程学习通过set_term_weight_for_envs修改这里，让同一term在不同env上发挥不同强度
    ...
    def compute(self, dt: float) -> dict[str, torch.Tensor]:
        ...
        """
        # 按combine_method初始化：sum清零累加，prod初始化为dt连乘
        for group_name in self.__group_term_cfgs.keys():
        ...
        for group_name, terms_cfgs in self.__group_term_cfgs.items():
            ...
            for term_name, term_cfg in zip(self.__group_term_names[group_name], terms_cfgs):
                ...
                # 优先使用per-env权重（课程学习动态调过），否则回退到cfg标量权重
    ...
    def set_term_weight_for_envs(
        ...
        """
        # 课程学习核心接口：按env_ids批量修改指定term的权重，实现按环境难度的差异化奖励
        if group_name is None:
            group_name = list(self.__group_term_names.keys())[0]
        if group_name not in self.__group_term_names:
            raise ValueError(f"Group '{group_name}' not found.")
        ...
        # 若per-env缓冲不存在则按cfg标量初始化整个env张量，再覆盖指定env_ids位置
        ...
        # 标量权重广播到所有env_ids；序列/tensor必须与env_ids长度一致
