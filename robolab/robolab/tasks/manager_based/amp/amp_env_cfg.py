# AMP环境配置：定义场景、动作、观测、奖励、终止、事件、动作数据等模块
# 关键设计：观测分policy/critic/discriminator/disc_demo四组，判别器组用于区分策略与参考动作
# 动作数据通过MotionDataTerm加载.pkl参考动作，AnimationTerm提供动画回放与采样
...
class CommandsCfg:
    ...
    # 速度指令配置：lin_vel_x/y、ang_vel_z、heading四项，由UniformVelocityCommand随机采样
...
class ObservationsCfg():
    # 观测配置：分policy/critic/discriminator/disc_demo四组
    # policy：策略观测，含噪声，history_length=3，用于actor网络
    # critic：critic观测，含特权信息（base_lin_vel），无噪声，history_length=3
    # discriminator：判别器策略侧观测（机器人状态），history_length=10，关键体位置由key_body_pos_b提供
    # disc_demo：判别器参考侧观测（参考动作），由AnimationTerm提供，与discriminator组对齐用于判别器训练
    ...
    class DiscriminatorCfg(ObsGroup):
        # 判别器策略侧观测：机器人当前状态，history_length=10
        # 与disc_demo对齐用于二分类，输出风格奖励
        # root_local_rot_tan_norm = ObsTerm(func=mdp.root_local_rot_tan_norm)
        # base_lin_vel = ObsTerm(func=mdp.base_lin_vel)
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel)
        joint_pos = ObsTerm(func=mdp.joint_pos)
    ...
    class DiscriminatorDemoCfg(ObsGroup):
        # 判别器参考侧观测：从AnimationTerm获取参考动作序列
        # 与discriminator组对齐，作为判别器正样本（专家示范）
...
class RewardsCfg:
    ...
    # 任务奖励：速度跟踪（线速度+角速度）+ 惩罚项（z速度、xy角速度、力矩、加速度、动作率、足部airtime）
    # AMP风格奖励由判别器单独提供，不在此处配置
...
class AmpEnvCfg(AnimationEnvCfg):
    ...
    # AMP环境主配置：组合场景、观测、动作、指令、奖励、终止、事件、课程、动作数据、动画
    # 默认num_envs=4096，decimation=4（控制频率=物理频率/4），episode_length_s=20s
